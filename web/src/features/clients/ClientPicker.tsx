...
export function ClientPicker({
  ...
  if (value) {
    return (
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-md border border-ink-200 bg-ink-50 p-4">
        <div>
          <p className="font-medium text-ink-900">{value.full_name}</p>
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {isOpen && (
        ...
        >
          {!search && (
            <p className="border-b border-ink-100 px-4 py-2 text-xs font-medium uppercase tracking-wide text-ink-400">
              Recently registered
            </p>
          )}
              ...
              {data.data.map((client) => (
                <li key={client.id}>
                  <button
                    ...
                    className="flex w-full items-center justify-between gap-4 px-4 py-2.5 text-left hover:bg-ink-50"
                  >
                    <span>
                    ...
                    <span className="whitespace-nowrap text-right text-sm">
                      {client.affordability ? (
                        <>
                          ...
                          <span className="block text-xs text-ink-500">free monthly</span>
                        </>
                      ) : (
                        <span className="text-xs text-warn-700">No assessment</span>
                      )}
                    </span>
                  </button>
                </li>
